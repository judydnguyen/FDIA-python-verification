...
def trades_loss(model,
    ...
    loss_natural = criterion(logits, y)
    # loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(x_adv), dim=1),
    #                                                 F.softmax(model(x_natural), dim=1), min=1e-8)
    
    loss_robust = (1.0 / batch_size) * criterion_kl(F.log_softmax(model(x_adv), dim=1),
                                                        torch.clamp(F.softmax(model(x_natural), dim=1), min=EPS))

    # print(f"loss_natural: {loss_natural}, \tloss_rb: {loss_robust}")
    # if loss_robust is torch.nan:
    # print(f"loss_natural: {loss_natural}, \tloss_rb: {loss_robust}")
